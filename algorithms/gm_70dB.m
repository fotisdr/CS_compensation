function [xp,En,g]=gm_70dB(x,cs,win_len,height,prominence,distance)
% The gm_70dB function processes an input stimulus based on the desired 
% cochlear-synaptopathy (CS) profile parameters and the modified 
% processing strategy, preserving the envelope modulation depth. The 
% envelope processing of a 70 dB-SPL stimulus is applied here, 
% irrespectively of the amplitude of the signal.
% 
...
if any(cs~=[13,3,3])
    ...
    for j=1:length(minima)-1
        ...
        for n=1:size(x_segment,1)
            ...
            else % if the Hilbert envelope was used omit the first and last 100 values
    ...
    % process the stimulus using the computed processing function gm
    xp=gm.*x;
else
